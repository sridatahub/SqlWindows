...
from ranked_sale
where ranking<=3;
--7.Percent Rank of Sales(0/1)
select sale_id,
	category,
	amount,
	percent_rank() over(partition by category order by amount desc) as perc_rank
from sales1;
--8.Calculate the difference between the current row’s amount and the average amount for its
select 
...
--ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING tells the database to consider all rows within the current partition (like all sales for a specific product category) when using a window function
select
	category,
	sale_date,
	amount,
